...
void updateShoulder(){
	float deadZone = 0.2;
	float slowZone = 0.6;
	uint8_t normShoulderSpeed = 0;
	if (!shoulderL && !shoulderR){
//          std::cout << "Update " << shoulderSpeed << std::endl;
            if (shoulderSpeed > slowZone){
                //write(fd,"U",1);
		write(fd,"A",1);
//		std::cout << "shoulderSpeed " << shoulderSpeed << std::endl;		
		normShoulderSpeed = 0;
//		std::cout << "normShoulderSpeed " << (int)normShoulderSpeed << std::endl;
		write(fd,&normShoulderSpeed,1);
	    }
	    else if (shoulderSpeed > deadZone){
		write(fd,"A",1);
		normShoulderSpeed = 100;
	  	write(fd,&normShoulderSpeed,1);
	    }
	    else if (shoulderSpeed < -slowZone){
		write(fd,"A",1);
	 	normShoulderSpeed = 255;
                write(fd,&normShoulderSpeed,1);	  	
	    }
	    else if (shoulderSpeed < -deadZone){
	 	write(fd,"A",1);
		normShoulderSpeed = 155;
                write(fd,&normShoulderSpeed,1);
	    }
            else
                write(fd,"S",1);
