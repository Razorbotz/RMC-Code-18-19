...
bool bin=false; //togeles bin dumping everything else (roll pitch yaw)
int8_t elbowDir = 1;
...
//ros::Publisher binStatePublisher;
...
//        std::cout << "theta " << theta.data << std::endl;
	...
	if(!drive && !bin){
		...
		elbowSpeed.data = -elbowSpeed.data;
...
void updateBin(){
	std_msgs::Float32 binSpeed;
	if(bin)
	    binSpeed.data=joystick1Pitch*0.2;
	else
	    binSpeed.data=0.0;
	binSpeedPublisher.publish(binSpeed);
}
...
//		std::cout << "BUTTON 8" << std::endl;
                break;
	    case 8: //toggles the bin state
		bin = !bin;
		updateBin();
                break;
	    case 9: //toggels the elbow direction
		if(buttonState.state)
		    elbowDir=elbowDir*-1;
                break;
	    case 10:
...
//	binStatePublisher=nodeHandle.advertise<std_msgs::Bool>("bin_state",1);
